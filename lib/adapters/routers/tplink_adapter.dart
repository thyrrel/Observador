// /lib/adapters/routers/tplink_adapter.dart
// ┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓
// ┃ 📡 TPLinkAdapter - Controle de roteadores TP-Link ┃
...
class TPLinkAdapter implements RouterAdapter {
  ...
  TPLinkAdapter({this.debugMode = false});
  ...
  Future<RouterSession?> login(String ip, String username, String password) async {
    try {
      ...
      if (resp.statusCode == 200 || resp.statusCode == 302) {
        ...
        if (cookie != null) {
          return RouterSession(token: cookie, type: RouterType.TPLink);
        }
      }
    } catch (e) {
      if (debugMode) print('[TPLinkAdapter] login error: $e');
    }
    return null;
  }
  ...
  Future<bool> unblockDevice(String ip, String token, String mac) async {
    final endpoints = ['http://$ip/userRpm/AccessCtrlAccessRulesRpm.htm?mac=$mac&block=0'];
    return _genericPost(endpoints, token, {});
  }

  @override
  Future<bool> limitDevice(String ip, String token, String mac, int limit) async {
    if (debugMode) print('[TPLinkAdapter] limitDevice not supported');
    return false; // TP-Link geralmente não expõe isso via API
  }

  @override
  Future<bool> removeLimit(String ip, String token, String mac) async {
    if (debugMode) print('[TPLinkAdapter] removeLimit not supported');
    return false;
  }
  ...
  Future<List<RouterDevice>> _genericClientsRequest(List<String> endpoints, String token) async {
    for (final ep in endpoints) {
      try {
        ...
        if (debugMode) print('[TPLinkAdapter] client fetch error: $e');
      }
    }
    return [];
  }
  ...
  Future<bool> _genericPost(List<String> endpoints, String token, Map<String, dynamic> body) async {
    for (final ep in endpoints) {
      try {
        ...
        if (debugMode) print('[TPLinkAdapter] post error: $e');
      }
    }
    return false;
  }
