// /lib/adapters/routers/ubiquiti_adapter.dart
// ┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓
// ┃ 📡 UbiquitiAdapter - Controle de roteadores Ubiquiti ┃
...
class UbiquitiAdapter implements RouterAdapter {
  final bool debugMode;
  UbiquitiAdapter({this.debugMode = false});

  @override
  Future<RouterSession?> login(String ip, String username, String password) async {
    try {
      final uri = Uri.parse('http://$ip/api/login');
      ...
      if (resp.statusCode == 200) {
        ...
        final token = data['token'] ?? 'ubnt_session';
        return RouterSession(token: token.toString(), type: RouterType.Ubiquiti);
      }
    } catch (e) {
      if (debugMode) print('[UbiquitiAdapter] login error: $e');
    }
    return null;
  }
  ...
  Future<List<RouterDevice>> _genericClientsRequest(List<String> endpoints, String token) async {
    for (final ep in endpoints) {
      try {
        ...
        if (debugMode) print('[UbiquitiAdapter] client fetch error: $e');
      }
    }
    return [];
  }
  ...
  Future<bool> _genericPost(List<String> endpoints, String token, Map<String, dynamic> body) async {
    for (final ep in endpoints) {
      try {
        ...
        if (debugMode) print('[UbiquitiAdapter] post error: $e');
      }
    }
    return false;
  }
